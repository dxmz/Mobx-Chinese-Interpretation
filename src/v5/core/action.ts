...
export function _startAction(actionName: string, scope: any, args?: IArguments): IActionRunInfo {
    ...
    // untrack globalState.trackingDerivation
    // 将当前的globalState.trackingDerivation保存为prev并返回，同时置为空值，
    // 即在 action 期间不进行依赖收集（get 代理）
    const prevDerivation = untrackedStart()
    startBatch()
    // allowStateChanges允许改变mobx对象
    // 设置configure({ enforceActions: 'always' }) 则必需在 action 下才能改变数据
    // enforceActions为'always'或'strict'时，allowStateChanges为false，
    // 只有在action中才会将allowStateChanges置为true，action结束后又将allowStateChanges恢复原值
    // observable执行set()时会调用checkIfStateModificationsAreAllowed()检查函数检查是否可以更改值
...
export function _endAction(runInfo: IActionRunInfo) {
    ...
    // endBatch() 中执行runReactions()
    // 并且会将pendingUnobservations(即observable.observers.size === 0)未被观察的对象置为未观察
    endBatch()
